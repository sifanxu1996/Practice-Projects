...
*/
%% DUPLICATES PROBLEM
prereqFor(graphics, PrereqsSorted) :-
  prereqForHelper(graphics, Prereqs),
  sort(Prereqs, PrereqsSorted).

prereqForHelper(graphics, [M, C1, C2, C3, C4, C5, A]) :-
  member(M, [math267, math277]),
  member(C1, [cpsc453]),
  member(C2, [cpsc587, cpsc589, cpsc591]),
  member(C3, [cpsc587, cpsc589, cpsc591]),
  C2 \= C3,
  member(C4, [cpsc481, cpsc535, cpsc585, cpsc587, cpsc589, cpsc591, cpsc599]),
  member(C5, [cpsc481, cpsc535, cpsc585, cpsc587, cpsc589, cpsc591, cpsc599]),
  C4 \= C5,
...
*/
containsAnti(AllReq_NoAnti) :-
  sort(AllReq_NoAnti, AllReq_NoAntiSorted),
  sublist(A, AllReq_NoAntiSorted),
  length(A, Alength),
  Alength == 2,
  antireqs(AntiList),
  sort(AntiList, AntiListSorted)
  sublist(A, AntiListSorted).
...
% neededCoursesCG([math267, cpsc587, cpsc481, art231], X).
...
% createSublist([2,3,4,5], [1,2,3], X).
% X = [2,3].
createSublist([], _, []).
createSublist([C|Cs], List, Sublist) :-
  member(C, List), !,
  createSublist(Cs, List, X),
  Sublist = [C|X].
createSublist([C|Cs], List, Sublist) :-
  \+ member(C, List),
  createSublist(Cs, List, Sublist).
